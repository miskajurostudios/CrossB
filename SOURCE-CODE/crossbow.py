...
# Canvas pro crosshair
canvas = tk.Canvas(root, width=size, height=size, highlightthickness=0, bg=bg_color)
canvas.pack()

def draw_crosshair(cross_type):
    canvas.delete("all")
    center = size // 2
    color = "#f9f9f9"
    width = 1

    if "Red" in cross_type:
        color = "red"
    elif "Blue" in cross_type:
        color = "#39a7f1"

    if "Thicker" in cross_type:
        width = 3

    if "Dot" in cross_type:
